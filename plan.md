...
The implemented expression/control-flow slice includes short-circuit boolean operations, scalar equality, numeric comparisons, `if`/`else` branches, exhaustive scalar `when` branches with literal cases and a required `else`, and native action loops. Callback and lifecycle action blocks support `for item in array`, `while condition`, `break`, and `continue`; these lower directly to the target language's native control flow. Enum pattern matching, loop expressions, and ranges remain future language slices. See [language design decisions](docs/language-design.md) for choices around the remaining Kotlin/Swift concepts.
