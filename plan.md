...
The current compiler performs a conservative IR pass that folds pure literal boolean/scalar/numeric expressions, removes unreachable UI and event branches, removes unused state and value-struct declarations, and tree-shakes pure functions that are unreachable from the app. It runs after semantic lowering and before either native backend, so it adds no runtime layer and keeps native component mappings unchanged. The compiler also reports unused declarations, function parameters, action-loop and `FastList` row bindings, pure functions, and constant conditions; `--deny-warnings` promotes those diagnostics to a failed check or build. See [compiler diagnostics and optimization](docs/compiler-diagnostics.md).
...
The first fixed-size optimization is implemented: `FastList(itemExtent: positiveNumber)` maps to native fixed row or cell dimensions for vertical, horizontal, and grid paths. Omitting `itemExtent` preserves dynamic native sizing; no shared measurement pass is introduced. The first pagination slice is also implemented with a trailing `onEndReached { ... }` action block. UIKit observes the last visible row through its native delegate, Compose observes `LazyListState` or `LazyGridState`, and each source count can trigger the callback only once until more rows are added. Direct `RefreshControl` children now attach native UIKit refresh controls to FastList table/collection views and use Compose `PullToRefreshBox` around lazy lists without replacing the virtualized path. `scrollPosition: Int32` tracks the first visible item and restores or moves the native list through UIKit scroll APIs or Compose lazy-list state. A trailing `onScroll { ... }` action block is now integrated and runs once per first-visible-row/item change, avoiding per-pixel callback dispatch. Swift feature-gates the event fields, forwarding arguments, and callback code so ordinary FastList output keeps the smaller runtime path; Compose emits the observer only when the event is used. A trailing `stickyHeader { ... }` block is now integrated for vertical lists, lowering to a pinned UIKit section header or Compose `stickyHeader`; horizontal and grid headers are rejected at compile time. The generated Swift host emits only the FastList axis runtimes used by the module, and the UIKit header identifier, closure storage, and delegate methods are emitted only when a module uses a sticky header, so ordinary FastList output keeps the smaller runtime path. See [list-item-extent.nx](examples/list-item-extent.nx), [list-pagination.nx](examples/list-pagination.nx), [list-refresh.nx](examples/list-refresh.nx), [list-scroll-position.nx](examples/list-scroll-position.nx), [list-scroll-events.nx](examples/list-scroll-events.nx), and [list-sticky-header.nx](examples/list-sticky-header.nx). Sectioned data is now integrated with `FastList(sections: ...)` over `Array<Array<T>>`: iOS emits a native sectioned `UITableView`, Android emits direct nested `LazyColumn` iteration, and both expose explicit section/index/item bindings, scalar keys, fixed extents, optional per-section headers, and direct refresh integration. Sectioned lists reject horizontal/grid axes, the ordinary sticky-header block, pagination, scroll events, and scroll-position binding until their semantics can remain section-native without a flattening collection. The sectioned runtime is feature-gated so apps without sectioned lists pay no generated code cost. See [list-sections.nx](examples/list-sections.nx) alongside [list-item-extent.nx](examples/list-item-extent.nx), [list-pagination.nx](examples/list-pagination.nx), [list-refresh.nx](examples/list-refresh.nx), [list-scroll-position.nx](examples/list-scroll-position.nx), [list-scroll-events.nx](examples/list-scroll-events.nx), and [list-sticky-header.nx](examples/list-sticky-header.nx).
...
The iOS `FastList` default is a native `UITableView` with reusable cells and SwiftUI row content hosted through `UIHostingConfiguration`. This keeps UIKit responsible for cell virtualization and reuse without creating an intermediate row array. The first stable-key slice accepts scalar `key` expressions over the index/item bindings and applies them to hosted row identity; Compose passes the same expression to `LazyColumn`'s native key slot. `axis: Horizontal` selects a native `UICollectionView` with horizontal flow layout on iOS and `LazyRow` on Android; `axis: Grid(columns)` selects a native compositional `UICollectionView` grid on iOS and `LazyVerticalGrid` on Android. The default `Vertical` path remains `UITableView`/`LazyColumn`. Sectioned list components may use different native collection primitives when those capabilities are implemented and measured.
