...
The implemented expression/control-flow slice includes short-circuit boolean operations, scalar equality, numeric comparisons, `if`/`else` branches, exhaustive scalar `when` branches with literal cases and a required `else`, and native action loops. Callback and lifecycle action blocks support `for item in array`, `for item in start..end`, positive literal range steps, `while condition`, `break`, and `continue`; these lower directly to the target language's native control flow, with Int32 ranges kept compact and unmaterialized. Enum pattern matching, loop expressions, dynamic range steps, and ranges outside `for` remain future language slices. See [language design decisions](docs/language-design.md) for choices around the remaining Kotlin/Swift concepts.
