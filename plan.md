...
The current compiler supports the scalar types above plus nullable `T?` values, `Array<T>`, `Set<T>`, `Map<K, V>`, `Pair<A, B>`, and `Triple<A, B, C>` as contextually typed value declarations and literals. Nullable values and `null` lower to native Swift optionals and Kotlin nullable types; `??` lowers to each platform's native fallback operator. Array indexing with an `Int32` index and map lookup with a declared key type are implemented as direct native subscripting; map lookup returns `V?`. Pair and triple field access uses compile-time checked `.first`, `.second`, and `.third` members with direct native tuple/standard-library access. Set elements and map keys are currently limited to scalar `String`, `Bool`, and numeric types for native hashability. Scalar `value in array`, `value in set`, and `key in map` lower to direct native membership checks. Action `for` loops iterate arrays, sets, and Int32 ranges directly; collection mutation, map iteration, and transformations remain roadmap work.
