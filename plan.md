...
The current compiler supports the scalar types above plus nullable `T?` values, `Array<T>`, `Set<T>`, `Map<K, V>`, `Pair<A, B>`, and `Triple<A, B, C>` as contextually typed value declarations and literals. Nullable values and `null` lower to native Swift optionals and Kotlin nullable types; `??` lowers to each platform's native fallback operator. Pair and triple field access uses compile-time checked `.first`, `.second`, and `.third` members with direct native tuple/standard-library access, including `?.` optional chaining that returns an optional field. Array indexing with an `Int32` index and map lookup with a declared key type are implemented as direct native subscripting; map lookup returns `V?`. Set elements and map keys are currently limited to scalar `String`, `Bool`, and numeric types for native hashability. Scalar `value in array`, `value in set`, and `key in map` lower to direct native membership checks. Action `for` loops iterate arrays, sets, and Int32 ranges directly; collection mutation, map iteration, and transformations remain roadmap work.
...
The implemented expression/control-flow slice includes short-circuit boolean operations, scalar equality, numeric comparisons, `if`/`else` branches, exhaustive scalar `when` branches with literal cases and a required `else`, and native action loops. Callback and lifecycle action blocks support `for item in array`, `for item in start..end`, `while condition`, `break`, and `continue`; these lower directly to the target language's native control flow, with Int32 ranges kept compact and unmaterialized. Enum pattern matching, loop expressions, explicit range steps, and ranges outside `for` remain future language slices. See [language design decisions](docs/language-design.md) for choices around the remaining Kotlin/Swift concepts.
...
The current compiler performs a conservative IR pass that folds pure literal boolean/scalar/numeric expressions, removes unreachable UI and event branches, removes unused state declarations, and tree-shakes pure functions that are unreachable from the app. It runs after semantic lowering and before either native backend, so it adds no runtime layer and keeps native component mappings unchanged. The compiler also reports unused declarations, pure functions, and constant conditions; `--deny-warnings` promotes those diagnostics to a failed check or build. See [compiler diagnostics and optimization](docs/compiler-diagnostics.md).
