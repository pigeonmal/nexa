...
if [[ $# -ne 3 ]]; then
    echo "usage: hot-reload-remote-image.sh <nexa> <ios|android> <project-dir>" >&2
    exit 2
fi
...
"$nexa" create RemoteImageSmoke --directory "$project"
cp "$script_dir/fixtures/dev_remote_image.nx" "$project/App.nx"
...
wait_for_patch() {
    ...
    cat "$log_file" >&2
    echo "timed out waiting for the remote-image patch" >&2
    return 1
}
patch_source() {
...
old = 'url: "https://www.gstatic.com/webp/gallery/1.webp",\n                description: "Remote image"'
new = 'url: "https://www.gstatic.com/webp/gallery/2.webp",\n                description: "Updated remote image"'
if old not in source:
    raise SystemExit("missing original remote image node")
...
if [[ "$platform" == android ]]; then
    xml="$project/build/remote-image.xml"
    wait_for_description() {
        local expected=$1
        for _ in $(seq 1 90); do
            adb shell uiautomator dump /sdcard/nexa-remote-image.xml >/dev/null 2>&1
            adb exec-out cat /sdcard/nexa-remote-image.xml >"$xml"
    ...
    wait_for_description "Remote image"
    patch_source
    wait_for_patch "$patch_count"
    wait_for_description "Updated remote image"
    if grep -Fq 'content-desc="Remote image"' "$xml"; then
        echo "stale remote image description remained after reload" >&2
        exit 1
    fi
    echo "Nexa Android remote-image hot reload passed."
    exit 0
fi

ios_project="$project/build/ios/RemoteImageSmoke.xcodeproj"
test_source="$project/build/ios/NexaHotReloadRemoteImageUITests.swift"
...
sed -e "s/__READY_TOKEN__/$ready_token/" \
    ...
    "$script_dir/ios-hot-reload-remote-image-ui.swift" >"$test_source"
test_runner_bundle_id=$(ruby "$script_dir/add-ios-ui-test-target.rb" "$ios_project" \
    NexaHotReloadRemoteImageUITests.swift NexaHotReloadRemoteImageUITests)
...
xcodebuild test -project "$ios_project" -scheme NexaHotReloadRemoteImageUITests \
...
wait_for_marker nexa-remote-image-ready "$ready_token" >/dev/null
patch_source
wait_for_patch "$patch_count"
container=$(xcrun simctl get_app_container "$simulator_id" "$test_runner_bundle_id" data)
printf '%s' "$patched_token" >"$container/tmp/nexa-remote-image-patched"
...
echo "Nexa iOS remote-image hot reload passed."
