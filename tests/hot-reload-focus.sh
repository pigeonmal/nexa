...
"$nexa" create FocusReloadSmoke --directory "$project"
...
wait_for_keyboard() {
    for _ in $(seq 1 60); do
        if [[ "$platform" == android ]]; then
            adb shell dumpsys input_method >"$artifacts/input-method.txt"
            if grep -Eq 'mInputShown=true|isInputViewShown=true|mShowRequested=true' "$artifacts/input-method.txt"; then
                return 0
            fi
        else
            xcrun simctl io booted screenshot "$artifacts/screen.png" >/dev/null 2>&1
            "$project/read-screenshot-text" "$artifacts/screen.png" >"$artifacts/screen-text.txt"
            if grep -Eiq 'space|return|123|emoji' "$artifacts/screen-text.txt"; then return 0; fi
        fi
        sleep 1
    done
    if [[ "$platform" == android ]]; then cat "$artifacts/input-method.txt" >&2; else cat "$artifacts/screen-text.txt" >&2; fi
    cat "$log_file" >&2
    echo "software keyboard was not visible on $platform" >&2
    return 1
}
...
focus_android_input() {
    for _ in $(seq 1 30); do
        ...
        adb exec-out cat /sdcard/nexa-window.xml >"$artifacts/window.xml"
        local coordinates
        coordinates=$(python3 - "$artifacts/window.xml" <<'PY'
import re
import sys
...
PY
    ...
    echo "could not find the editable TextInput in the Android UI tree" >&2
    return 1
}
...
wait_for_patch_after() {
    ...
    cat "$log_file" >&2
    echo "timed out waiting for the runtime to apply a focus-preserving patch" >&2
    return 1
}

wait_for_log "Nexa $platform_label dev runtime connected."
wait_for_log "Nexa $platform_label dev runtime applied module"
wait_for_visible_text "Focus Ready"
if [[ "$platform" == android ]]; then focus_android_input; fi
wait_for_keyboard
...
old = 'Text("Focus footer")'
if old not in source:
    raise SystemExit("focus smoke footer was not found")
path.write_text(source.replace(old, 'Text("Focus after reload")', 1))
PY

wait_for_patch_after "$patch_count"
wait_for_visible_text "Focus after reload"
wait_for_keyboard
echo "Nexa $platform focused-input retention and hot reload passed."
