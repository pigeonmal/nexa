...
if [[ $# -ne 3 ]]; then
    echo "usage: hot-reload-image.sh <nexa> <ios|android> <project-dir>" >&2
    exit 2
fi
...
fixture="$script_dir/fixtures/dev_image.nx"
...
"$nexa" create ImageSmoke --directory "$project"
cp "$fixture" "$project/App.nx"
mkdir -p "$project/assets/images"
python3 - "$project/assets/images/sample_photo.png" <<'PY'
...
width = height = 24
row = b"\x00" + bytes((40, 130, 230, 255)) * width
pixels = row * height
png = b"\x89PNG\r\n\x1a\n"
...
wait_for_patch() {
    ...
    cat "$log_file" >&2
    echo "timed out waiting for the image-node patch" >&2
    return 1
}
patch_source() {
...
old = 'description: "Fitted image", scale: Fit'
new = 'description: "Updated fitted image", scale: Fill'
if old not in source:
    raise SystemExit("missing original image node")
...
if not match: raise SystemExit("generated Android applicationId was not found")
print(match.group(1))
PY
)
    app_res="$project/build/android/app/src/main/res/drawable-nodpi/sample_photo.png"
    test -f "$app_res" || { echo "Android app image resource was not generated" >&2; exit 1; }
    xml="$project/build/image.xml"
    wait_for_description() {
        local expected=$1
        for _ in $(seq 1 90); do
            adb shell uiautomator dump /sdcard/nexa-image.xml >/dev/null 2>&1
            adb exec-out cat /sdcard/nexa-image.xml >"$xml"
            if grep -Fq "content-desc=\"$expected\"" "$xml"; then return 0; fi
            sleep 1
        done
        cat "$xml" >&2
        return 1
    }
    wait_for_description "Fitted image"
    wait_for_description "Filled image"
    patch_source
    wait_for_patch "$patch_count"
    wait_for_description "Updated fitted image"
    if grep -Fq 'content-desc="Fitted image"' "$xml"; then
        echo "stale image accessibility description remained after reload" >&2
        exit 1
    fi
    echo "Nexa Android bundled image assets and image-node hot reload passed."
    exit 0
fi

ios_project="$project/build/ios/ImageSmoke.xcodeproj"
test_source="$project/build/ios/NexaHotReloadImageUITests.swift"
...
sed -e "s/__READY_TOKEN__/$ready_token/" \
    ...
    "$script_dir/ios-hot-reload-image-ui.swift" >"$test_source"
test_runner_bundle_id=$(ruby "$script_dir/add-ios-ui-test-target.rb" "$ios_project" \
    NexaHotReloadImageUITests.swift NexaHotReloadImageUITests)
...
xcodebuild test -project "$ios_project" -scheme NexaHotReloadImageUITests \
...
container=$(wait_for_marker nexa-image-ready "$ready_token")
ios_image="$project/build/ios/ImageSmoke/Assets.xcassets/sample_photo.imageset/sample_photo.png"
test -f "$ios_image" || { echo "iOS app image asset was not generated" >&2; exit 1; }
patch_source
wait_for_patch "$patch_count"
container=$(xcrun simctl get_app_container "$simulator_id" "$test_runner_bundle_id" data)
printf '%s' "$patched_token" >"$container/tmp/nexa-image-patched"
...
echo "Nexa iOS bundled image assets and image-node hot reload passed."
