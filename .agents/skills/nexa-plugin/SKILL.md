...
- The supported workflow is `nexa plugin init <plugin.id> --out <directory> --name <TypeName>`, followed by `nexa plugin check <plugin-directory|interfaces.nxid>` and `nexa plugin generate <plugin-directory|interfaces.nxid> --target <swift|kotlin>`. Pass `--package` for the generated Kotlin package. The scaffold includes `interfaces.nxid`; its parser validates typed models, methods, reserved-name collisions, declared type references, generic arity, compile-time config options, and asynchronous `Result<Success, Failure>` boundaries. Binding generation emits direct Swift protocols or Kotlin interfaces and native model declarations. A local `.nx` app can declare `plugin "path" as Namespace` and call `await Namespace.method(...)`; the compiler resolves the IDL statically and generated projects include the plugin's conventional iOS/Android source trees. Dependency resolution, package installation, generated implementation methods, and typed error recovery remain future work.
