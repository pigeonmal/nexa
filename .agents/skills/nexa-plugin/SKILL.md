...
- The supported workflow is `nexa plugin init <plugin.id> --out <directory> --name <TypeName>`, followed by `nexa plugin check <plugin-directory|interfaces.nxid>` and `nexa plugin generate <plugin-directory|interfaces.nxid> --target <swift|kotlin>`. Pass `--package` for the generated Kotlin package. The scaffold includes `interfaces.nxid`; its parser validates typed models, methods, generic types, duplicate names, and asynchronous `Result<Success, Failure>` boundaries. Binding generation emits direct Swift protocols or Kotlin interfaces and native model declarations. Typed `.nx` call sites, dependency resolution, and installation remain future work.
