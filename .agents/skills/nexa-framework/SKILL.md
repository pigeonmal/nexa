...
- Keep the authoring surface small: expose one vertical `Column` layout and infer `let` or `state` declarations when a non-empty initializer has one deterministic type. Integer literals default to `Int32` and decimal literals to `Float64`; component parameters stay explicit. These are compiler-only conveniences and must preserve direct generated native output.
