...
- Keep `OnAppear { ... }` as one top-level app callback with direct assignment/`if` actions. Lower it to SwiftUI `.onAppear` and Compose `LaunchedEffect(Unit)`; do not add a shared lifecycle registry or runtime event bus. Reject nested/repeated declarations until screen and app lifecycle semantics are defined.
