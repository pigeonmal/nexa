...
- Use `Text(value, fontWeight: Normal|Medium|Semibold|Bold, lineLimit: positiveInt, lineHeight: number, letterSpacing: number)` for native text emphasis, metrics, and static truncation. Swift and Compose receive direct native text styling; custom spans and selectable text are not part of the current syntax.
...
- Use at most one top-level `OnAppear { ... }` and one `OnDisappear { ... }` block in the app body or each named screen. Put direct state assignments or `if` actions in the blocks; Swift uses lifecycle modifiers and Android uses `LaunchedEffect(Unit)`/`DisposableEffect(Unit)`. App active/background events and async lifecycle work are not part of the current syntax.
