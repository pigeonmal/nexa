...
- Use at most one top-level `OnAppear { ... }` block in the app body or each named screen. Put direct state assignments or `if` actions in the block; Swift uses `.onAppear` and Android uses `LaunchedEffect(Unit)`. App active/background events and async lifecycle work are not part of the current syntax.
