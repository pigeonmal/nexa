...
- Use one top-level `OnAppear { ... }` block for app appearance work. Put direct state assignments or `if` actions in the block; Swift uses `.onAppear` and Android uses `LaunchedEffect(Unit)`. Screen-specific appearance/disappearance, app active/background events, and async lifecycle work are not part of the current syntax.
