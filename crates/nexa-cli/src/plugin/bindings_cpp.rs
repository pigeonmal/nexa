...
pub(crate) fn render_android_adapters(
    idl: &PluginIdl,
    plugin_id: &str,
...
#[cfg(test)]
mod tests {
    use super::{render, render_android_adapters, render_swift_adapters};
    ...
    fn cpp_contract_maps_native_values_and_typed_async_errors() {
        ...
        assert!(
            cpp.contains("virtual void setOnEnded(std::function<void()> handler) noexcept = 0;")
        );
    ...
    fn swift_cpp_byte_alias_avoids_plugin_type_names() {
        ...
        assert!(
            swift.contains(
                "plugin_dev.plugin_example.plugin_cpp_dash_plugin.NexaCppByteBuffer2(data)"
            )
        );
    }
    ...
    fn swift_cpp_adapters_bridge_async_scalar_string_and_byte_methods() {
        ...
        assert!(
            adapters.contains("public final class CounterPlugin: Counter, @unchecked Sendable")
        );
        assert!(
            adapters
                .contains("plugin_dev.plugin_example.plugin_cpp_dash_plugin.Counter.echo(value)")
        );
    ...
    fn swift_cpp_adapters_bridge_compatible_sets_through_vector_facades() {
        ...
        assert!(
            cpp.contains(
                "std::set<std::int32_t> integers(std::set<std::int32_t> values) noexcept;"
            )
        );
        ...
        assert!(
            collision_cpp
                .contains("nexaSwiftAdapter_transform1(NexaCppArrayInt32 values) noexcept")
        );
    ...
    fn android_cpp_adapters_generate_typed_kotlin_jni_and_owned_native_classes() {
        ...
        assert!(
            jni.contains("Java_dev_example_app_NexaPlugin2_1CppBindings_service_1Counter_1echo")
        );
        assert!(jni.contains(
        ...
        assert!(
            kotlin.contains("external fun service_Counter_echoBytes(value: ByteArray): ByteArray")
        );
        assert!(
    ...
    fn android_cpp_adapters_reject_unsafe_shapes_and_bridge_flat_primitive_arrays() {
        let no_dispose = nexa_plugin_idl::parse("native class Meter { init() }")
            .expect("native IDL should parse");
        ...
        assert!(
            jni.contains("std::bit_cast<jlong>(static_cast<std::uint64_t>(*nexaOptionalValue))")
        );

        let collection = nexa_plugin_idl::parse(
            "service Lookup { fn find(ids: Array<UInt32>) -> Array<UInt32> }",
        )
        ...
        assert!(
            kotlin.contains(
                "external fun service_Lookup_find(ids: Array<IntArray>): Array<IntArray>"
            )
        );
        ...
        assert!(
            kotlin.contains(
                "external fun service_Lookup_strings(values: Array<String>): Array<String>"
            )
        );
        assert!(kotlin.contains(
        ...
        assert!(
            kotlin.contains("external fun service_Lookup_integers(values: IntArray): IntArray")
        );
        ...
        let signed_set = nexa_plugin_idl::parse(
            "service Lookup { fn signed(values: Set<Int32>) -> Set<Int32> }",
        )
