...
Usage:\n  nexa check <source.nx> [--deny-warnings]\n  nexa build <source.nx> --target <swift|kotlin> [--out <path>] [--deny-warnings]\n  nexa audit <source.nx> [--target <ios|android|all>] [--release-sizes] [--out <path>]\n  nexa generate <source.nx> [--target <ios|android|all>] [--out <directory>] [--name <AppName>] [--deny-warnings]\n  nexa plugin init <plugin.id> [--kind <pure|native>] [--out <directory>] [--name <TypeName>] [--version <version>]\n  nexa plugin check <plugin-directory|native.nxid>\n  nexa plugin generate <plugin-directory|native.nxid> --target <swift|kotlin|cpp> [--package <kotlin.package>] [--out <file>]\n\n\
Targets emit native SwiftUI or Jetpack Compose source. `generate` creates a self-contained native project bundle. `plugin init --kind pure` creates a source-only Nexa package; the default native kind creates a `plugin.config.nx`/`native.nxid` optional-plugin scaffold; `plugin check` validates its package manifest and typed native contract, and `plugin generate` emits direct Swift/Kotlin binding skeletons."
    );
}
