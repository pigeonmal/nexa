...
| Lifecycle appearance | Integrate first slice | One top-level `OnAppear { ... }` action block is supported on the app body and named screens, lowering directly to SwiftUI `.onAppear` or Compose `LaunchedEffect(Unit)`; app background events remain deferred. |
