...
| Text input submit | Integrate first slice | An optional `TextInput` action block lowers to SwiftUI `.onSubmit` and Compose `KeyboardActions` with a native Done IME action; fields without a block keep their existing direct output. Focus, selection, autofill, and submit error handling remain deferred. |
