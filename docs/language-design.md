...
| Dictionary / Map | Lookup slice integrated | `Map<K, V>` lowers to Swift `Dictionary<K, V>` and Kotlin read-only `Map<K, V>`. Keys are scalar `String`, `Bool`, or numeric values; map literals use last-value-wins for duplicate keys. `map[key]` returns `V?` through direct native lookup; mutation and membership helpers remain deferred. |
