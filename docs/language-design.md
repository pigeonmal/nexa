...
| Pressable long press | Integrate first slice | An optional third `Pressable` action block lowers to SwiftUI `.onLongPressGesture` and Compose `combinedClickable`; the two-block form keeps the direct tap-only path. Hover, focus, haptics, and custom accessibility actions remain deferred. |
