...
| Text weight and line limits | Integrate first slice | `Text` maps `fontWeight` and `lineLimit` directly to SwiftUI modifiers and Compose `fontWeight`/`maxLines`; custom text layout and nested spans remain deferred. |
