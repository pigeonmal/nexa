...
| Lifecycle appearance | Integrate first slice | One top-level `OnAppear { ... }` action block lowers directly to SwiftUI `.onAppear` or Compose `LaunchedEffect(Unit)`; screen-specific and app background events remain deferred. |
