...
- `Text(expression, color: ..., fontSize: ..., fontWeight: ..., lineLimit: ...)` accepts strings, booleans, and numeric values. Text color accepts a hexadecimal color or theme color token. Font size uses points on iOS and scale-independent pixels on Android. `fontWeight` accepts `Normal`, `Medium`, `Semibold`, or `Bold`; `lineLimit` is a positive integer and maps to native line truncation. See [text-style.nx](../examples/text-style.nx).
