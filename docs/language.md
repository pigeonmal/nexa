...
- `Stack { ... }` is the overlay container. It maps to SwiftUI `ZStack` and Compose `Box`; children are emitted once in source order, and static `alignment`, size, background, border, and other layout styles use the same native modifier path. `spacing` has no effect on an overlay container. See [stack.nx](../examples/stack.nx).
