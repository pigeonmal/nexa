...
`for` currently iterates an `Array<T>` directly, and its binding is an immutable value with the array element type. `while` requires a `Bool` condition. `break` and `continue` are valid only inside one of these loops. Loop actions are accepted in button, pressable, text-input submit, refresh, and lifecycle callbacks and lower directly to Swift `for`/`while` and Kotlin `for`/`while`; no iterator, callback registry, or shared loop runtime is generated. Functions remain pure and continue to reject loops. Use `FastList` for repeated UI rows so virtualization is preserved. See [action-loops.nx](../examples/action-loops.nx).
