...
- `screen Home { ... }` declares a named destination. A screen may declare `state` or `let` bindings before its UI nodes; those bindings are scoped to that screen and lower to direct native storage. An app with screens uses `body { NavigationStack(root: Home) }`; `NavigationLink(destination: Profile) { ... }` pushes a statically resolved screen. Screen state names must be unique across the app. Destinations must be declared in the same app.
