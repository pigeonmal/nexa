...
Swift receives a private value `struct` with a direct initializer; Kotlin receives a private `data class`. Struct calls are lowered through private typed helpers, with no runtime model registry or boxed field access. `user?.name` is supported for optional structs and returns an optional field. Recursive value layouts, named constructor arguments, methods, inheritance, and mutation are not supported.
