...
plugin implementation. IDL `throws` and `Result<Success, Failure>` declarations
are parsed and emitted in native contracts. Swift contracts use typed throws;
Kotlin contracts use `@Throws` metadata and sealed error classes. Async throwing
calls in action blocks require an explicit `try { ... } catch { ... }` block.
Catch arms use `case Namespace.Error.variant(payload) { ... }`, and `else { ... }`
provides a catch-all. Stateless service calls lower to their direct native
singleton (`CameraPlugin.shared` on Swift or `CameraPlugin.instance` on
Kotlin); native class constructors and methods target concrete object values.
`nexa generate` includes the plugin's local platform source tree in the
generated project. Plugin-only modules do not emit the core Network/Path/File
helper library or Android Cronet dependencies.
...
component implementation directly. Declared component events can be attached
with `.on<Event> { ... }` modifiers after the call; payload bindings use the
same explicit closure form as native-class events. Child content blocks,
package installation, and dynamic plugin lookup remain future work.
