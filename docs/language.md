...
- `FastList(count: rowCount, axis: Vertical|Horizontal, index: row, key: row) { ... }` creates a virtualized list of `Int32` row indices. `axis` defaults to `Vertical`; vertical rows use iOS `UITableView` or Android `LazyColumn`, while horizontal rows use iOS `UICollectionView` or Android `LazyRow`. `index` is optional and defaults to `index`; `key` is optional and may use the index binding. The count can be an `Int32` state value; negative runtime counts are treated as zero.
- `FastList(items: labels, axis: Vertical|Horizontal, index: row, item: label, key: label) { ... }` creates a virtualized list over an `Array<T>` state. The index and item bindings are optional and default to `index` and `item`; both are read-only, and the item has the array's element type. An optional `key` expression may use either binding and must resolve to scalar `String`, `Bool`, or numeric type. Swift applies the key to hosted cell identity and Compose passes it to the native lazy-list key slot; without a key, rows use their current position. The collection is indexed directly without building an intermediate row array or row tree. `Set<T>` and `Map<K, V>` are values but are not accepted as list sources. See [collection-list.nx](../examples/collection-list.nx) and [horizontal-list.nx](../examples/horizontal-list.nx).
