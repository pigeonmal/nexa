...
The compiler currently supports stateful native screens, typed value structs, typed arrays and collection values, virtualized lists, conditional UI and event control flow, compile-time platform blocks, a parameterless native navigation stack, stateful user-defined components, native status bars, sheets, pull-to-refresh, labeled application bottom bars with static icons and badges, typed external links, accessibility semantics, static RTL/LTR direction, typed app-local async lifecycle work, local typed plugin calls, and a feature-gated native network/file library. The entry `.nx` file declares one `app`; imported component files can declare reusable components and top-level structs without an app.
...
Non-escaping collection transformations are available with inline closures: `values.map { value -> expression }`, `values.filter { value -> condition }`, and `values.reduce(initial) { accumulator, value -> expression }`. The compiler checks closure arity, parameter bindings, result types, and the `Bool` result required by `filter`; `Array<T>` is currently the supported source collection. Swift lowers directly to `map`, `filter`, and `reduce`, while Kotlin uses `map`, `filter`, and `fold` with no Nexa runtime callback or boxed collection wrapper. `await` is rejected inside these synchronous callbacks. See [collection-transform.nx](../examples/collection-transform.nx). `FastList(items: ...)` is the current way to render an `Array<T>` as repeated rows. See [collection-values.nx](../examples/collection-values.nx), [collection-indexing.nx](../examples/collection-indexing.nx), [collection-membership.nx](../examples/collection-membership.nx), [collection-mutation.nx](../examples/collection-mutation.nx), and [map-iteration.nx](../examples/map-iteration.nx).
...
- `Pressable(disabled: Bool, haptic: Light|Medium|Heavy) { ... } { ... } { ... }` wraps composed content in a native accessible press target. Its second block contains tap actions and its optional third block contains long-press actions; `disabled` accepts a Boolean expression and defaults to `false`, while `haptic` is an optional static native feedback style. Swift emits a direct `Button` with `.disabled(...)`, UIKit impact feedback, and `.onLongPressGesture`; Compose passes the expression to `combinedClickable` or `clickable` and calls `performHapticFeedback`. Omitting haptic and the third block keeps the smaller path, and a static `false` adds no disabled modifier. See [pressable-long-press.nx](../examples/pressable-long-press.nx).
...
- `Accessibility(label: String, role: Button|Link|Header|Image) { ... }` adds an accessible label and optional native role to any child subtree. The label accepts any `String` expression, including state, interpolation, and function results; literal labels must be non-empty. Swift emits native accessibility modifiers and Android emits Compose semantics, importing the native `role` property only when a matching role is used; `Header` uses heading semantics on Android. Hints, focus control, custom actions, and dynamic accessibility values beyond the label remain future work. See [accessibility.nx](../examples/accessibility.nx).
- `FastList(count: rowCount, axis: Vertical|Horizontal|Grid(columns), itemExtent: positiveNumber, index: row, key: row, scrollPosition: position) { ... }` creates a virtualized list of `Int32` row indices. `axis` defaults to `Vertical`; vertical rows use iOS `UITableView` or Android `LazyColumn`, horizontal rows use iOS `UICollectionView` or Android `LazyRow`, and `Grid(columns)` uses a native compositional grid or `LazyVerticalGrid`. `columns` and `itemExtent` must be positive literals. `itemExtent` enables fixed native sizing: it sets row height for vertical lists, square cell dimensions for horizontal lists, and cell height for grids; omit it to retain dynamic native sizing. `index` is optional and defaults to `index`; `key` is optional and may use the index binding. `scrollPosition` is optional and must be a mutable `Int32` state binding; it tracks the first visible row and applies external changes through native scroll APIs. The count can be an `Int32` state value; negative runtime counts are treated as zero. Add a trailing `onEndReached { ... }` action block to load another page when the last visible row is reached, a trailing `onScroll { ... }` action block to react once per first-visible-row change, or a trailing `stickyHeader { ... }` block to render a native sticky section header. Sticky headers are valid only for vertical lists; pagination and scroll callbacks do not run once per pixel, and the end callback re-arms when the source count changes.
- `FastList(items: labels, axis: Vertical|Horizontal|Grid(columns), itemExtent: positiveNumber, index: row, item: label, key: label, scrollPosition: position) { ... }` creates a virtualized list over an `Array<T>` state. The index and item bindings are optional and default to `index` and `item`; both are read-only, and the item has the array's element type. An optional `key` expression may use either binding and must resolve to scalar `String`, `Bool`, or numeric type. Swift applies the key to hosted cell identity and Compose passes it to the native lazy-list key slot; without a key, rows use their current position. `scrollPosition` is optional and must be a mutable `Int32` state binding; native paths report the first visible row/item when it changes and apply external index changes without an animation. The collection is indexed directly without building an intermediate row array or row tree. `Set<T>` and `Map<K, V>` are values but are not accepted as list sources. A trailing `onEndReached { ... }` block receives no row binding and can update mutable state directly. A trailing `onScroll { ... }` block receives no row binding and runs once per first-visible-row/item change on UIKit or Compose lazy state. A trailing `stickyHeader { ... }` block is lowered to a UIKit section header or Compose `stickyHeader` and is accepted only with `axis: Vertical`. When a `FastList` is the direct child of `RefreshControl`, refresh actions are attached to the native list path without an intermediate list wrapper. See [collection-list.nx](../examples/collection-list.nx), [horizontal-list.nx](../examples/horizontal-list.nx), [grid-list.nx](../examples/grid-list.nx), [list-item-extent.nx](../examples/list-item-extent.nx), [list-pagination.nx](../examples/list-pagination.nx), [list-refresh.nx](../examples/list-refresh.nx), [list-scroll-position.nx](../examples/list-scroll-position.nx), [list-scroll-events.nx](../examples/list-scroll-events.nx), and [list-sticky-header.nx](../examples/list-sticky-header.nx). Sectioned data uses `FastList(sections: groups, section: sectionIndex, index: itemIndex, item: item, key: item) { ... } sectionHeader { ... }` with an `Array<Array<T>>` binding. The `section` and `index` values are read-only `Int32` bindings, and `item` has the nested array element type. iOS emits a native sectioned `UITableView`; Android emits a `LazyColumn` that iterates each section directly. `sectionHeader` is optional and maps to native table headers or Compose `stickyHeader`; `itemExtent`, scalar keys, and direct `RefreshControl` are supported. Sections currently reject `axis` values other than `Vertical`, the ordinary `stickyHeader` block, `scrollPosition`, `onEndReached`, and `onScroll` so the compiler never creates a flattened temporary collection. See [collection-list.nx](../examples/collection-list.nx), [list-sections.nx](../examples/list-sections.nx), [horizontal-list.nx](../examples/horizontal-list.nx), [grid-list.nx](../examples/grid-list.nx), [list-item-extent.nx](../examples/list-item-extent.nx), [list-pagination.nx](../examples/list-pagination.nx), [list-refresh.nx](../examples/list-refresh.nx), [list-scroll-position.nx](../examples/list-scroll-position.nx), [list-scroll-events.nx](../examples/list-scroll-events.nx), and [list-sticky-header.nx](../examples/list-sticky-header.nx).

## Compile-time permissions

For `FastList`, the generated iOS runtime uses self-sizing estimated rows and cells by default. A positive literal `itemExtent` switches vertical rows to a fixed `UITableView` height, horizontal cells to a fixed `UICollectionView` cell size, and grid cells to a fixed compositional height; Compose receives matching native `Modifier` constraints. A vertical `stickyHeader` uses the table's native section-header pinning and Compose's lazy-list sticky-header API. Sectioned lists use native table sections on iOS and direct nested `LazyColumn` iteration on Android; their optional `sectionHeader` block is the per-section header API. Omit `itemExtent` when rows need dynamic sizing.
...
The iOS implementation configures `URLCache` with 16 MiB memory and 64 MiB disk capacity and uses `URLRequest.CachePolicy.useProtocolCachePolicy` by default. Optional certificate pins use the current Security trust-chain API. The Android implementation configures the Play Services Cronet engine with a 64 MiB disk cache, HTTP/2, QUIC, and Brotli; its private callback client is wrapped directly in suspend functions without a request registry. File reads and writes use native asynchronous APIs (`Task.detached` on iOS and `Dispatchers.IO` on Android). `NexaPath` provides `documents`, `caches`, `temporary`, `appSupport`, and `join` helpers.

The generated helpers are feature-gated to modules that use a remote image or one of these typed native calls. Offline apps remain free of networking imports and helper code. See [native-apis.nx](../examples/native-apis.nx) for a complete source example.
