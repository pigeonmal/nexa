...
- `Accessibility(label: String, role: Button|Link|Header|Image) { ... }` adds an accessible label and optional native role to any child subtree. The label accepts any `String` expression, including state, interpolation, and function results; literal labels must be non-empty. Swift emits native accessibility modifiers and Android emits Compose semantics; `Header` uses heading semantics on Android. Hints, focus control, custom actions, and dynamic accessibility values beyond the label remain future work. See [accessibility.nx](../examples/accessibility.nx).
