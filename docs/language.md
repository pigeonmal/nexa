...
Function parameters and the return type are explicit. The first function slice requires exactly one `return` expression and has no local variables, closures, async behavior, or generic functions. Calls are checked for name, arity, argument types, and return type during semantic lowering, then become direct private top-level Swift or Kotlin functions. There is no runtime function registry or dynamic dispatch. Function parameters are ordinary typed bindings, so the same `Int32` and `Float64` literal defaults apply when a numeric literal has no expected narrower type. See [functions.nx](../examples/functions.nx).
...
- `OnAppear { ... }` and `OnDisappear { ... }` run lifecycle callbacks directly on the app root or on a named screen. Each scope accepts at most one top-level block with the same direct state assignments and `if` actions as button handlers. Swift emits `.onAppear`/`.onDisappear`; Android emits `LaunchedEffect(Unit)`/`DisposableEffect(Unit)` with `onDispose`. Nested declarations are rejected, so callbacks attach without a lifecycle registry or runtime event bus. App active/background events and asynchronous cancellation remain future lifecycle slices. See [lifecycle.nx](../examples/lifecycle.nx) and [navigation.nx](../examples/navigation.nx).
...
- `Pressable(disabled: false) { ... } { ... } { ... }` wraps composed content in a native accessible press target. Its second block contains tap actions and its optional third block contains long-press actions; `disabled` is optional. Swift emits a direct `Button` with `.onLongPressGesture`, while Compose uses `combinedClickable` only when the third block is present. Omitting the third block keeps the smaller single-gesture path. See [pressable-long-press.nx](../examples/pressable-long-press.nx).
