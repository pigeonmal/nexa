...
- `OnAppear { ... }` runs an appearance callback directly on the app root or on a named screen. Each scope accepts at most one top-level block with the same direct state assignments and `if` actions as button handlers. Swift emits `.onAppear`; Android emits `LaunchedEffect(Unit)`. Nested declarations are rejected, so callbacks attach without a lifecycle registry or runtime event bus. App active/background events and asynchronous cancellation remain future lifecycle slices. See [lifecycle.nx](../examples/lifecycle.nx) and [navigation.nx](../examples/navigation.nx).
