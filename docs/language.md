...
- `OnAppear { ... }` runs one app-level appearance callback. The block accepts the same direct state assignments and `if` actions as button handlers. Swift emits `.onAppear`; Android emits `LaunchedEffect(Unit)`. It must appear once at the app body's top level, so the compiler can attach it without a lifecycle registry or runtime event bus. Screen-specific callbacks, app active/background events, and asynchronous cancellation remain future lifecycle slices. See [lifecycle.nx](../examples/lifecycle.nx).
