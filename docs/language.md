...
The generated helpers are feature-gated to modules that use a remote image or one of these typed native calls. Path/file-only apps do not receive the Cronet or URLSession network helper, and offline apps remain free of networking imports, dependencies, and helper code. Android coroutines are added only when generated helper code for asynchronous network/file operations or permission requests references them. See [native-apis.nx](../examples/native-apis.nx) for a complete source example.
