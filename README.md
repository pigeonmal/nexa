...
Optional Pair/Triple members support native safe access such as `maybePair?.first`, and optional arrays/maps support `values?[index]`, with `??` for a fallback. Swift and Kotlin keep their native optional chaining semantics; general user-defined optional members remain deferred. See [optional-members.nx](examples/optional-members.nx).
