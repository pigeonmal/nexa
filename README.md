...
`FastList` accepts `axis: Vertical|Horizontal|Grid(columns)`, an optional positive-literal `itemExtent`, and a trailing `onEndReached { ... }` action block for native infinite scrolling. Vertical lists use iOS `UITableView` or Compose `LazyColumn`; horizontal lists use iOS `UICollectionView` or Compose `LazyRow`; grids use a native compositional `UICollectionView` or `LazyVerticalGrid`. Fixed extents map directly to native row or cell sizing, while omitted extents retain dynamic sizing. The end callback is guarded once per source count and re-arms when pagination adds rows. Column counts are positive integer literals, and all paths keep native virtualization and direct scalar keys. See [horizontal-list.nx](examples/horizontal-list.nx), [grid-list.nx](examples/grid-list.nx), [list-item-extent.nx](examples/list-item-extent.nx), and [list-pagination.nx](examples/list-pagination.nx).
