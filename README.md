...
`FastList` accepts `axis: Vertical|Horizontal|Grid(columns)` and an optional positive-literal `itemExtent`. Vertical lists use iOS `UITableView` or Compose `LazyColumn`; horizontal lists use iOS `UICollectionView` or Compose `LazyRow`; grids use a native compositional `UICollectionView` or `LazyVerticalGrid`. Fixed extents map directly to native row or cell sizing, while omitted extents retain dynamic sizing. Column counts are positive integer literals, and all paths keep native virtualization and direct scalar keys. See [horizontal-list.nx](examples/horizontal-list.nx), [grid-list.nx](examples/grid-list.nx), and [list-item-extent.nx](examples/list-item-extent.nx).
