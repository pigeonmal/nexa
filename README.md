...
`FastList` accepts `axis: Vertical|Horizontal|Grid(columns)`, an optional positive-literal `itemExtent`, a mutable `scrollPosition: Int32` binding for native restoration and programmatic scrolling, a trailing `onEndReached { ... }` action block for native infinite scrolling, a trailing `onScroll { ... }` action block, and a trailing `stickyHeader { ... }` block for vertical lists. `onScroll` fires once when the first visible row/item changes, so native scrolling does not invoke Nexa actions for every pixel. Native pull-to-refresh is available when a FastList is placed directly inside `RefreshControl`. Vertical lists use iOS `UITableView` or Compose `LazyColumn`; horizontal lists use iOS `UICollectionView` or Compose `LazyRow`; grids use a native compositional `UICollectionView` or `LazyVerticalGrid`. Swift generation emits only the FastList axis runtimes used by the module. Fixed extents map directly to native row or cell sizing, while omitted extents retain dynamic sizing. The scroll position binding tracks the first visible item and applies external changes through native scroll APIs. The end callback is guarded once per source count and re-arms when pagination adds rows. Sticky headers map to UIKit section headers and Compose `stickyHeader`; the compiler rejects them for horizontal and grid lists. Sectioned lists use `FastList(sections: groups, section: sectionIndex, index: itemIndex, item: item, key: item) { ... } sectionHeader { ... }` with an `Array<Array<T>>` source. iOS keeps native `UITableView` sections and reusable cells; Android emits one `LazyColumn` with direct nested section iteration and optional sticky headers. Sectioned lists support `itemExtent`, scalar keys, section/item bindings, headers, and direct refresh integration; the first slice intentionally rejects pagination, scroll callbacks, scroll-position binding, and the ordinary `stickyHeader` block so no flattened intermediate collection is generated. Direct refresh uses `UIRefreshControl` on UIKit and `PullToRefreshBox` around the Compose lazy list; other refresh content keeps the generic native wrapper. Column counts are positive integer literals, and all paths keep native virtualization and direct scalar keys. See [horizontal-list.nx](examples/horizontal-list.nx), [grid-list.nx](examples/grid-list.nx), [list-item-extent.nx](examples/list-item-extent.nx), [list-pagination.nx](examples/list-pagination.nx), [list-refresh.nx](examples/list-refresh.nx), [list-scroll-position.nx](examples/list-scroll-position.nx), [list-scroll-events.nx](examples/list-scroll-events.nx), [list-sticky-header.nx](examples/list-sticky-header.nx), and [list-sections.nx](examples/list-sections.nx).
...
`FastList` accepts an optional scalar `key` expression for stable row identity, such as `FastList(items: products, item: product, key: product.id)`. The key is checked at compile time and maps to UIKit hosted-row identity and Compose `LazyColumn` keys without materializing another collection.
