...
Nexa is alpha software. Its language and generated native contracts intentionally have no legacy aliases or compatibility shims; when a design changes, the current spelling and architecture are updated directly across the compiler, generated code, examples, and documentation.

The current implementation covers the compiler foundation and a growing native-control slice. It parses app state, typed value structs, checks primitive and collection value types and bindings, lowers to a platform-independent IR, and emits native SwiftUI or Compose controls, including virtualized range and collection lists. `nexa generate` also creates a native iOS Xcode 27 project with Swift 6 settings and an Android Gradle project using AGP 9.2.1, Kotlin 2.4.20, and API 37 around the generated sources, including a typed `nexa.config.nx` file for compile-time host metadata.
...
cargo run -p nexa-cli -- check examples/collection-mutation.nx
cargo run -p nexa-cli -- check examples/collection-transform.nx
cargo run -p nexa-cli -- check examples/collection-equality.nx
...
cargo run -p nexa-cli -- build examples/collection-mutation.nx --target swift --out /tmp/CollectionMutation.swift
cargo run -p nexa-cli -- build examples/collection-mutation.nx --target kotlin --out /tmp/CollectionMutation.kt
cargo run -p nexa-cli -- build examples/collection-transform.nx --target swift --out /tmp/CollectionTransform.swift
cargo run -p nexa-cli -- build examples/collection-transform.nx --target kotlin --out /tmp/CollectionTransform.kt
...
Plugins declare their compile-time option schema in `native.nxid`, and the
...
Compiler orchestration and semantic analysis are separate modules inside `nexa-compiler`. Each native backend keeps its generator in three responsibility folders: `api/` owns optional native API emitters such as networking, permissions, paths, and files; `components/` owns one UI feature per file; and `engine/` owns shared lowering primitives and orchestration. A feature module declares the imports required by the source it emits. The backend `engine/imports.rs` module only collects those contributions, filters duplicates, applies deterministic ordering, and renders the final import block. A shared IR walker keeps structural scans consistent across backends; each backend analyzes a module once and uses that result to emit only the imports and native helpers the generated app needs. Multi-target project generation loads and parses the imported source graph once, then lowers the retained AST independently for Swift and Kotlin. This keeps platform concerns out of the shared IR and makes compiler changes easier to review and maintain.
...
User-defined `.nx` components can live in imported files, declare typed inputs and private state, compose other components, use `NavigationLink`, and expose one `Content()` slot for trailing content blocks. They compile directly into native SwiftUI or Compose declarations. Each reachable component has one generated native declaration, and every use calls that declaration directly; imports resolve relative to the source file and are statically compiled, while unreachable component declarations are omitted from generated output. See [custom-component-slot.nx](examples/custom-component-slot.nx) and [custom-component-navigation.nx](examples/custom-component-navigation.nx).

Remote images use the generated network stack: URLSession with a 16 MiB memory/64 MiB disk URLCache on iOS, and Coil 3 backed by a Cronet client with a 64 MiB disk cache, HTTP/2, QUIC, and Brotli on Android. Network-only apps use the same Cronet core without the Coil adapter or image dependencies. Image-only modules emit only a private native transport and omit the public `NexaNetwork` request API; Android also uses a GET-only image client without upload or file-sink request code. `NexaPath` and `NexaFile` are emitted only when their qualified calls are used. See the [network and file API guide](docs/language.md#native-network-and-file-library) for request options, downloads, cache behavior, and certificate pinning.
...
`StatusBar(style: Default|Light|Dark, hidden: true|false, background: "#RRGGBB")` is a static app-root or named-screen configuration that lowers to native status-bar APIs; see [status-bar.nx](examples/status-bar.nx) and [navigation.nx](examples/navigation.nx).

`Pressable` supports a content block followed by an explicit `.onPress { ... }` action and optional `.onLongPress { ... }` action, plus a Boolean `disabled` expression and static `haptic: Light|Medium|Heavy` feedback. Long press, disabled state, and haptics map to native SwiftUI/UIKit and Compose APIs without a shared runtime; see [pressable-long-press.nx](examples/pressable-long-press.nx).
...
`FastList` uses one explicit source followed by named options and an explicit row binding block. Use `FastList(rows, key: .self, rowHeight: 56) { row, index in ... }`; use `FastList(count: rowCount) { index in ... }` for generated rows; use `FastList(sections: groups, key: .self) { item, index, section in ... }.sectionHeader { ... }` for grouped data. `key` accepts `.self` or one member path such as `.id`, and `rowHeight` is a positive static dimension. `axis`, `scrollPosition`, and the optional dot modifiers `.onEndReached { ... }`, `.onScroll { ... }`, `.stickyHeader { ... }`, and `.sectionHeader { ... }` remain compile-time options. The row binding count is fixed by the source (one for `count`, two for an array, three for sections), so item/index/section names cannot be silently inferred or collide. Vertical lists use iOS `UITableView` or Compose `LazyColumn`; horizontal lists use iOS `UICollectionView` or Compose `LazyRow`; grids use a native compositional `UICollectionView` or `LazyVerticalGrid`. The compiler rejects legacy aliases such as `items`, `itemExtent`, `id`, `item`, `index`, and bare trailing modifier words. Native pull-to-refresh is available when a FastList is placed directly inside `RefreshControl`; generated code keeps only the axis and features used by the module. See [syntax-audit.md](docs/syntax-audit.md) and the FastList examples.
...
Mutable collection states use direct native mutation: arrays support `append(value)` and `remove(index)`, sets support `insert(value)` and `remove(value)`, and maps support `set(key, value)` and `remove(key)`. The compiler checks method names, argument counts, and element/key/index types. Swift emits native collection methods; Android uses Compose snapshot list/set/map collections so updates recompose without rebuilding a collection. See [collection-mutation.nx](examples/collection-mutation.nx).
Collection transformations use inline closures over arrays: `values.map { value -> expression }`, `values.filter { value -> condition }`, and `values.reduce(initial) { accumulator, value -> expression }`. The compiler checks closure types and emits direct Swift/Kotlin collection operations (`fold` for Kotlin reductions). See [collection-transform.nx](examples/collection-transform.nx).

Stable identity is written as a list-specific key path: `FastList(products, key: .id) { product, index in ... }` or `FastList(rows, key: .self) { row, index in ... }`. The compiler resolves the path against the explicit row binding, checks that it is a scalar hashable type, and lowers it directly to UIKit hosted-row identity or Compose lazy-list keys without materializing another collection. See [syntax-audit.md](docs/syntax-audit.md).
