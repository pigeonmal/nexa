...
User-defined `.nx` components can live in imported files, declare typed inputs and private state, compose other components, and expose one `Content()` slot for trailing content blocks. They compile directly into native SwiftUI or Compose declarations. Each reachable component has one generated native declaration, and every use calls that declaration directly; imports resolve relative to the source file and are statically compiled, while unreachable component declarations are omitted from generated output. See [custom-component-slot.nx](examples/custom-component-slot.nx).
